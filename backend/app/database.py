...
from app.config import get_settings, resolve_sqlite_database_url

settings = get_settings()
logger = logging.getLogger(__name__)

DATABASE_URL = resolve_sqlite_database_url(settings.DATABASE_URL)
...
def invalidate_pool() -> None:
    """Drop pooled connections after a dropped DB socket."""
    try:
        engine.dispose()
    except Exception as exc:
...
def with_db_retry(fn, /, *, retries: int = _DB_RETRY_ATTEMPTS):
    """Run a DB write callback in a fresh session, retrying on transient errors."""
